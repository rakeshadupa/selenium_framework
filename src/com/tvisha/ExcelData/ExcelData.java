...
public class ExcelData {
	...
	public ExcelData(String path) {
		...
		//=================for particular coloum with TC ID
	...
	public static Object[][] hashtableGetData1(String hash_Sheet,String testMode){
		...
		for (int rNum = 2; rNum<= getRowCount(hash_Sheet); rNum++)
		{
	  ...
	  /* int hash_row=getRowCount(hash_Sheet);	
	   ...
	   for(int i=1;i<hash_row;i++){
		   ...
		   hastable.put("SheetName",hash_Sheet); 
		    ...
		    hastable.put(getCellData(hash_Sheet, 2, 0), getCellData(hash_Sheet, 2, i));
		  System.out.println("key==> "+getCellData(hash_Sheet, 2, 0) + " Value===>"+ getCellData(hash_Sheet, 2, i));
	  ...
	  data=new Object[row_count][2];
	 //  data=new Object[hash_row-1][1];
	 
	   //data=new Object[hash_row-1][2];
	...
	//==================OR==========
	public static Object[][] hashtableGetData(String hash_Sheet,String testName){
		Object data[][]=null;
		...
		if(testName!=null && !testName.isEmpty()){
			 ...
			 for (int rNum = 1; rNum<= getRowCount(hash_Sheet); rNum++) {
				if (getCellData_col_Name(hash_Sheet, "TC"
						+ "ID", rNum).equalsIgnoreCase(testName)) {
					testStartRow = rNum;
					System.out.println("Test Case " + testName+ " starts from Row Number " + testStartRow + "in sheet" + hash_Sheet);
					break;
				}
			}			
			if (testStartRow >= 0){
				hastable = new Hashtable<String, String>();
				hastable.put("SheetName", hash_Sheet);
				for(int col = 0; col <hash_col; col++) {
					hastable.put(getCellData(hash_Sheet, col, 1), getCellData(hash_Sheet, col, testStartRow));
					System.out.print(getCellData(hash_Sheet, col, testStartRow) +" ");
				}
				/* It will put the Data into the Table for Each row and then 
				 * put the table inside the Two dimensional Array
				*/
		...
		int hash_row=getRowCount(hash_Sheet);	
	  // int hash_row=sheet.getLastRowNum();
	   System.out.println("hash_row "+hash_row);
	   int hash_col=sheet.getRow(0).getLastCellNum();
	   System.out.println("hash_col "+hash_col);
	   ...
	   for(int i=1;i<hash_row;i++){
		   ...
		   hastable=new Hashtable<>();
		   hastable.put("SheetName",hash_Sheet); 
		   String run_mode = hastable.put(getCellData(hash_Sheet, 2, i), getCellData(hash_Sheet, 2, i));
